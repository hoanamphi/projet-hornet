...
if (hornet_js_utils_1.Utils.config.getOrDefault("mock.enabled", false) && hornet_js_utils_1.Utils.config.getOrDefault("mock.serviceData.enabled", false)) {
    Promise.all([
        Promise.resolve().then(function () { return require("./mock/services/data/auth/auth-service-impl-mock"); })
    ]).then(function (_a) {
    ...
    Promise.all([
        Promise.resolve().then(function () { return require("./services/data/auth/auth-service-impl"); })
    ]).then(function (_a) {
